    ...
    $(document).ready(function() {
      ...
      function updateTotal() {
        ...
        $('.subtotal').each(function() {
          ...
          let subtotalValue = parseFloat(subtotalText.replace(/฿|,/g, '')); // Convert to float
          if (!isNaN(subtotalValue)) {
            total += subtotalValue; // Only add if it's a valid number
          }
        });

        $('#total-amount').text(total.toFixed(2)); // Update hidden total for checkout
      }
