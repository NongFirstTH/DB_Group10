...
<form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
  ...
  <div>
    ...
    <x-text-input id="email" name="email" type="email"
      class="mt-1 block w-full border border-gray-300 p-2 bg-gray-200 rounded-md focus:border-transparent focus:ring-0"
      :value="old('email', auth()->user()->email)" required autocomplete="username" readonly />
    ...
    <div>
      <p class="text-sm mt-2 text-gray-600">
        ...
        <button form="send-verification"
          class="underline text-sm text-gray-600 hover:text-orange-600 focus:outline-none">
  ...
  <div class="flex items-center gap-4 mt-6">
    <button class="text-white px-4 py-2 rounded-md bg-orange-600 hover:bg-orange-500">
      {{ __('Save') }}
    </button>
