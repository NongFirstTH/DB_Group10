...
if (Auth::user() != null) {
    ...
    ?>
      ...
      <a href="{{route('profile.show-profile')}}"
        class="ml-2 text-gray-800 dark:text-white font-medium">{{ Auth::user()->username }}</a>
      @endauth
      @else
      ...
      <a href="{{ route('login') }}"
        class="text-white bg-orange-600 hover:bg-orange-700 focus:ring-4 focus:outline-none focus:ring-orange-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-orange-500 dark:hover:bg-orange-600 dark:focus:ring-orange-800 transition">
        Login
      </a>
      @endif
    </div>
