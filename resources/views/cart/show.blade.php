  ...
  <div class="container mx-auto flex flex-col lg:flex-row gap-8 px-4 md:px-5 lg:px-6">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 border-t border-gray-200 py-4">
        ...
        <div class="flex items-center flex-col lg:flex-row w-full gap-4">
          ...
          <form action="{{ route('cart.update', $item->id) }}" method="POST" class="flex items-center w-full justify-center gap-2">
            ...
            <!-- Minus Button -->
            <button name="action" value="decrease" type="submit" class="decrease-quantity group rounded-l-full px-4 py-2 border border-gray-200 flex items-center justify-center shadow-sm transition-all duration-300 hover:bg-gray-50 hover:border-gray-300">
              <svg class="stroke-gray-900 transition-all duration-300 group-hover:stroke-black" xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 22 22">
                <path d="M16.5 11H5.5" stroke-width="1.6" stroke-linecap="round" />
              </svg>
            </button>

            <!-- Quantity Number -->
            <input type="text" name="quantity" class="quantity-input border border-gray-200 outline-none text-gray-900 font-semibold text-sm w-full max-w-[118px] placeholder:text-gray-900 py-2 text-center bg-transparent" min="1" value="{{$item->quantity}}">

            <!-- Plus Button -->
            <button name="action" value="increase" type="submit" class="increase-quantity group rounded-r-full px-4 py-2 border border-gray-200 flex items-center justify-center shadow-sm transition-all duration-300 hover:bg-gray-50 hover:border-gray-300">
              <svg class="stroke-gray-900 transition-all duration-300 group-hover:stroke-black" xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 22 22">
          ...
          <form action="{{ route('cart.destroy', $item->id) }}" method="POST" class="flex items-center">
            ...
            <button type="submit" class="text-red-600 font-semibold flex items-center gap-1 hover:text-red-800 transition duration-300">
    ...
    <div class="bg-gray-50 rounded-lg p-6 w-full max-w-md">
      ...
      <form action="{{ route('cart.checkout') }}" method="POST" class="flex items-center justify-center mt-4">
        ...
        <button id="checkout-button" type="submit" class="rounded-full w-full py-4 text-center bg-orange-600 font-semibold text-sm text-white flex justify-center items-center transition-all duration-300 hover:bg-orange-700">
          Checkout
